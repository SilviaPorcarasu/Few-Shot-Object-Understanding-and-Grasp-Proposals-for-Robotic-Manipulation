#!/usr/bin/env python3
"""
Open the interactive top-K heatmap comparison viewer for every primary case.
...
def main() -> int:
    ...
    for idx, case_dir in enumerate(cases, start=1):
        ...
        result = subprocess.run(
            [
                sys.executable,
                str(CODE_ROOT / "visualize_heatmap_topk_side_by_side.py"),
                "--case-dir",
                str(case_dir),
                "--topk",
                str(topk),
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"visualize_heatmap_topk_side_by_side.py failed for {case_dir} "
